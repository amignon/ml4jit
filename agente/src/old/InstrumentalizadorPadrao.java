...
public class InstrumentalizadorPadrao implements ClassFileTransformer {
  ...
  public InstrumentalizadorPadrao() {
  ...
  public InstrumentalizadorPadrao(File f) {
    if (init) {
      return;
    }    
    init = true;    
    initSkip(f);
    if(Config.CONTADOR) {
      modificador = new ModificadorContador();
    }
    else {
      modificador = new ModificadorTempoCodigo();
    }
  }
  ...
  /**
   * Chama o modificador de método para a contagem de execuções.
   * @param m Método a ser modificado.
   * @throws Exception
   */
  private void processarContador(CtMethod m) throws Exception {   
    Copiador cop = new Copiador();
    CtMethod n = cop.copiar(m, m.getDeclaringClass());
    if(n != null) {
      modificador.modificar(n);
    }
    modificador.modificar(m);
  }
  
  private void processar (CtMethod m, CtClass c) throws Exception {
    if (Config.AMOSTRA) {
      Copiador cop = new Copiador();
      CtMethod n = cop.copiar(m, m.getDeclaringClass());
      if(n != null) {
