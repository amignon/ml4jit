# -*- coding: utf-8 -*-
"""
Created on Fri Feb 24 14:50:41 2017

Classes que representam as configuracoes da RVM para a execucao dos experimentos
com ML.
...
class ExperimentoLab01(object):   
    def comandoExp (self, comando, ml):
        comando.append("-X:aos:enable_recompilation=false")
        comando.append("-X:aos:initial_compiler=opt")
        comando.append("-X:irc:O0")
        if ml == False:
            #comando.append("-X:opt:treinamento=true")
            comando.append("-X:opt:selecionador=agente.ml.SelecionadorNull")
        else:
            comando.append("-X:opt:selecionador=agente.ml.SelecionadorML")
        # desabilita as otimizacoes de inline
        comando.append("-X:opt:inline=false")
        comando.append("-X:opt:inline_guarded=false")
        comando.append("-X:opt:inline_guarded_interfaces=false")
        comando.append("-X:opt:inline_preex=false")
...
class ExperimentoLab02(object):   
    def comandoExp (self, comando, ml):
        comando.append("-X:aos:enable_recompilation=false")
        comando.append("-X:aos:initial_compiler=opt")
        comando.append("-X:irc:O0")
        if ml == True:
            comando.append("-X:opt:selecionador=lta.rvm.SelecionadorML")
        # desabilita as otimizacoes de inline
        comando.append("-X:opt:inline=false")
        comando.append("-X:opt:inline_guarded=false")
        comando.append("-X:opt:inline_guarded_interfaces=false")
